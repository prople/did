...
pub fn verify_from_json(data: impl ToJSON, encoded: String) -> Result<(), DIDError> {
    ...
    let (decoded_base, decoded_bytes) =
        multibase::decode(encoded.clone()).map_err(|err| DIDError::HashLinkError(err.to_string()))?;
...
mod tests {
    ...
    fn test_verify_hl_invalid() {
        ...
        assert!(verified_hl.is_err());
        assert!(matches!(verified_hl.clone().unwrap_err(), DIDError::HashLinkError(_)));
        assert_eq!("error hashlink: invalid hashlink".to_owned(), verified_hl.unwrap_err().to_string())
    }
}
